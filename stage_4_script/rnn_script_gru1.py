...
if 1:
    ...
    if torch.cuda.device_count() > 1:
        ...
        model = Method_RNN(input_size, hidden_size, num_classes,10,device)
        model.to(device)
        #get ready for distributed training
        method_obj = torch.nn.parallel.DistributedDataParallel(model, device_ids=[local_rank],output_device=local_rank)
