...
class Method_RNN(method, nn.Module):
    ...
    def __init__(self, input_size, hidden_size, output_size, num_layers):
        ...
        self.rnn = nn.LSTM(input_size, hidden_size, batch_first=True, num_layers=10) ##try with LSTM and GRU
        # self.rnn_layers = nn.ModuleList([nn.RNN(input_size, hidden_size, batch_first=True) for _ in range(num_layers)])

        self.fc = nn.Linear(hidden_size, output_size) 
        
    def forward(self, x, prev_state):
        # if prev_state[0].shape != (10,64,256):
        #     print("bad one")
        #     prev_state[0] = torch.zeros(10, 64, self.hidden_size).cuda()
        # print(h0.size())
    ...
    def train(self, X, y):
        ...
        for epoch in range(self.max_epoch): # you can do an early stop if self.max_epoch is too much...
            ...
            state_c = torch.zeros(10, 64, self.hidden_size).cuda()
            state_h = torch.zeros(10, 64, self.hidden_size).cuda()
            state_c,state_h = state_c.cuda(),state_h.cuda()
            for batch_X, batch_y in train_dataloader:
                ...
                train_loss = loss_function(y_pred, y_true) #y_pred.transpose(1, 2)
